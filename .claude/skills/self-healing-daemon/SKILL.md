...
10. **Workspace prep for downstream gates** (only on `status == "passed"`):
    ...
    c. If `{target_dir}/.git` does not exist, run (as absolute paths, no `cd`):
       ...
       ```
       The `-b main` flag pins the initial branch name to `main` so the publish gate's default-branch logic works deterministically regardless of host `init.defaultBranch` config. Defensive — the pipeline workspace normally already starts on `main` (the Planner's `git checkout -b feature/...` is preceded by an init that lands on main); this locks the convention so future workflows can't regress.
    d. Touch `HEARTBEAT`.
