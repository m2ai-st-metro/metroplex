...
def _make_valid_agent_spec(title: str) -> str:
    ...
    lines = [
        ...
        "## Safety constraints",
        "",
        "- **main_skill**: keyword matching must be negation-aware (\"not X\" /",
        "  \"didn't X\" must not trigger positive match) and use word-boundary",
        "  regex (`\\bX\\b`) to avoid fragment collisions (\"limp\" in \"limpid\").",
        "  Forbidden silent failures: ambiguous inputs must NOT default; log",
        "  and clarify. Test pair: positive + negation case for each trigger.",
        "",
        "## Success criteria",
        "",
